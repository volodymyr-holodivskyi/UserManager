...
function sliceArray(array, page, pageSize) {
    ...
    return array.slice(pageNumber * pageSizeNumber, pageNumber * pageSizeNumber + pageSizeNumber);
}
...
async function addUser(req, res) {
    ...
    const role = 'customer';
    const entitlements = 'can_view_users';
    const created_at = dateTransform(new Date().toLocaleString('en-GB'));
    const updated_at = dateTransform(new Date().toLocaleString('en-GB'));

    return userService.addUser(name, email, hashPassword, role, created_at, updated_at, entitlements)
...
function editUser(req, res) {
    ...
    let { name, email, password,entitlements } = req.body;
    ...
    userService.getUserById(id)
        .then(rows => {
            ...
            if (entitlements === undefined || entitlements === '') {
            ...
            console.log(bcrypt.compareSync(password,'1234'));
